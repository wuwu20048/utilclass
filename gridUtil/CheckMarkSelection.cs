...
namespace NurseTemp.NusUtil
{
    public class GridCheckMarksSelection
    {
        ...
        public void SelectAll()
        {
            ...
            if (dataSource != null && dataSource.Count == view.DataRowCount)//换一种选择方式，list里是datarow
            {
                for (int i = 0; i < view.DataRowCount; i++)  // slow
                    selection.Add(view.GetDataRow(i));
            }
	        else
		        for ( int i = 0; i < view.DataRowCount; i++ )  // slow
        ...
        public void selectAllExecptFod() {
            selection.Clear();
            ICollection dataSource = view.DataSource as ICollection;
            //if (dataSource != null && dataSource.Count == view.DataRowCount)
            //    selection.AddRange(dataSource);  // fast
		...
		{
            if (IsRowSelected(rowHandle) == select) return;//list中是否已存在
			//object row = view.GetRow(rowHandle);
            DataRow[] grouprows = getGroupRow(rowHandle);
            if (select)
            {
        ...
        private DataRow[] getGroupRow(int rowHandle)
        {
            DataRow curRow = view.GetDataRow(rowHandle);
            string setno = curRow["setno"].ToString();
            DataView dataView = (DataView)view.DataSource;
            // view.get
            DataTable orderContent = dataView.Table;
            DataRow[] groupRow;
            if (!string.IsNullOrEmpty(setno) && setno != "0")
            {
                groupRow = orderContent.Select(string.Format("setno={0}", setno));

            }
            else
            {
                groupRow = new DataRow[1];
                groupRow[0] = curRow;
            }
            return groupRow;
		...
		public bool IsRowSelected(int rowHandle)
		{
			...
			//object row1 = view.GetRow(rowHandle);
		...
		private void view_CustomUnboundColumnData(object sender, CustomColumnDataEventArgs e)
		{
			...
			{
				if ( e.IsGetData )
                    e.Value = IsRowSelected(View.GetRowHandle(e.ListSourceRowIndex));//是否已在集合中
				else
                    SelectRow(View.GetRowHandle(e.ListSourceRowIndex), (bool)e.Value);//加入或移除集合
			}
		}
